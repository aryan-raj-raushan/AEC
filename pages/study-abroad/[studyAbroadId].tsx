...
const StudyAbroadOverView: React.FC<any> = () => {
	...
	const handleTab = (value: string) => {
		...
		setCurrentTab(value);
		router?.push(`/study-abroad/${countryId}/${value?.replace(/\s+/g, '-')?.toLowerCase()}`)
	...
	return (
		<RootLayouts>
					...
					<div className="absolute inset-0 text-white flex gap-4 max-w-screen-xl mx-auto px-4 my-6">
						<div className="flex flex-col gap-2">
							...
							<Button
								...
								fontSize="text-xs"
								icon={<Image src={CameraIcon} width={16} height={16} alt="" />}
							/>
						</div>
						<div className="flex flex-col gap-3 w-full">
							...
							<div className="flex gap-4 items-center">
								<h1 className="font-bold text-3xl">
									Study in {StudyAbroadData?.country_name}: Find Top Colleges,
			...
			<section className="mainSection max-w-screen-xl mx-auto">
				<PageDetailsTab TabsData={TabData} page={StudyAbroadData} currentTab={currentTab} />
			</section>
					</>
				  )
			}
